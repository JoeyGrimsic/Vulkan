...
\textbf{SPIR-V}: Is the intermediate form of slang? \\
\textbf{Slang}: Is a cool shader language that vulkan uses instead of glsl?\\
...
\textbf{Virtual Function}: Is this related to virtual memory? What
does keyword virtual do?\\
...
\textbf{Dynamic Rendering}: idk\\
\textbf{Max Concurrent Frames}: idk\\
\textbf{Memory Barriers}: idk\\
\textbf{Depth Image}: idk\\
\textbf{Z Buffer/ Depth Buffer}: idk\\
\textbf{Attachment}: idk, i think they are apart of frame buffers?\\
\textbf{Vk Queue Present KHR}: idk\\
\textbf{Device Idle}: idk\\
\textbf{Application Object}: idk\\
\textbf{Device-Local}: idk\\
\textbf{Swap Chain}: The idea of rendering to a frame and then
swapping it to be displayed\\
\textbf{Viewport}: \\
\textbf{In-Flight Frame}: idk\\
...
\textbf{Command Pools}: idk\\
\textbf{Ticket Number}: idk\\
\textbf{Subresource}: idk\\
