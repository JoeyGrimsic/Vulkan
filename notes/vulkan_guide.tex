...
\subsection*{\texttt{*{\_}CreateInfo and *{\_}Info}}
...
*_Info
\end{verbatim}
We are using some kind of struct\textemdash but I'm not sure what the
struct is for. The struct probably holds whatever these two kinds of
pointers point to?
\\\\
So actually, the \texttt{*} in \texttt{*\_CreateInfo} and
\texttt{*\_Info} represent the wildcard operator, and is just to show
the naming convention in the documentation. These names refer to
structs that have state information for Vulkan.

\subsection*{Commands}
Commands are the main part of Vulkan. Commands are how we talk
directly to the GPU.
\\\\
Commands are normally stored in a \textbf{command buffer}, so we
...
\subsection*{Vulkan 'Views' + Reading Code Snippet}
...
\begin{itemize}
  ...
  \item What what does 'CI' mean in 'depthStencilViewCI'?
\end{itemize}
