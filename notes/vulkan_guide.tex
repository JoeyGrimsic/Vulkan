...
\subsection*{The Draw Calls}\addcontentsline{toc}{subsection}{The Draw Calls}
\par So there are three kinds of draw calls:

\begin{enumerate}
  \item We have the light-weight, used for ui and other basic stuff
    \textit{vkCmdDraw()}
  \item We have the bread and butter, used for 3d models and 99\% of
    rendering \textit{vkCmdDrawIndexed()}
  \item The mysterious, advanced, and nuanced
    \textit{vkCmdDrawIndexedIndirect()}
\end{enumerate}

\subsection*{The Render Queue}\addcontentsline{toc}{subsection}{The
Render Queue}
\par So one of the important "bridges" between a game and Vulkan is
the concept of the \textit{Render Queue}. In the game, there will be
objects that have properties, like their x,y,z position, and their rotation.

The idea of the render queue is that it has a method called
submit(obj), where the render queue unpacks the object and it's state
to send to be turned into a draw call

\subsection*{Entity Component
System}\addcontentsline{toc}{subsection}{Entity Component System}
\par So I guess the idea of the the entity component system is that
we don't store data as objects, but as an index in a group of arrays.
The arrays each hold piece of data that the objects have; an example
would be an array of model matrices. The reason we do this is because
of CPU cache coherence. When the renderer is submitting the draw
calls, if we used the struct of arrays approach, then we can get a
performance bonus. The bonus comes from the fact that we don't need
to render unecessary data (in the context of rendering we don't need
to know an entity's health)

\subsection*{Particle System}\addcontentsline{toc}{subsection}{Particle System}
\par What happens if we are doing a physics simulation and we want a
ton of small objects? The problem is that if we decide to add them to
our entity component system, we could be adding hundreds or thousands
of object id's. One effect of this is that we would do culling checks
for all of them individually. Overall, our render queue has
a lot more work to do. This is expensive and suboptimal.

\par Instead, we treat the physics simulation as one entity, with a big
...
\par A struct is defined like the following:
\begin{verbatim}
struct MyStruct{
int a;
int b;
...
\par To use an instance of that struct, we can initialize like (requires c++20):
\begin{verbatim}
MyStruct myStruct{
.a = 1;
.b = 2;
...
\par Note that in this way we are initializing its members at the
same time as the struct, but we could initialize after the fact like this:
\begin{verbatim}
MyStruct myStruct{};
myStruct.a = 1;
myStruct.b = 2;
...
Headers}\addcontentsline{toc}{subsection}{Understanding Headers}
\par So actually headers are pretty simple.

\par If you want to have code that is global, in the sense that it is
reusable across different files, then it goes in a header.

\par On the contrary, if you just wanted a local helper function
local to the file it is implemented in, then you do not need to store
its signature in a header. \textit{The caveat to this is that if you
do not put the signature in a header (.h) file, then all of the calls
to the function must be after its definiton.} If you don't want to
put the function signature in a header (.h) file, and want to call it before
the function definiton, then you need the function signature at the
top of the cpp file that you defined it for.

\par In other words, if you want to call a function from another file
than where it was defined, you put the function signature in a header
and include it. This is to tell the compiler that this function
exists somewhere in the project if not here.

\par This is not the only purpose of headers, though. If we want
other functionality to extend beyond the scope of a single+isolated
cpp file, we need to include those things in the header. Here are
some more examples:
...
\item Type Aliases
\item Constants - if we wanted, we could inline a constant so that it
  is defined in the header?
\end{itemize}

\subsection*{Module}\addcontentsline{toc}{subsection}{Module}
\par What is a module in CPP? Should I use them?

\subsection*{RAII}\addcontentsline{toc}{subsection}{RAII}
\par RAII is the idea of writing a class with a destructor, and
attaching it to a smart pointer (Or some other stack-allocated
object). This way, when the smart pointer goes out of scope, the
resources are deallocated automatically, without manual delete calls.

\subsection*{Header Only
Development}\addcontentsline{toc}{subsection}{Header Only Development}
\par A funny but maybe powerful way of using cpp is implementing all
of your classes in .hpp files. The compiler treats them as inline
functions, so that they can be included into a cpp file and just work.

\subsection*{Factory Pattern}\addcontentsline{toc}{subsection}{Factory Pattern}
\par So in pursuit of abstracting the instancing sample in
vulkan-samples, one of the first abstractions is working with VkBuffer.

\par I don't fully understand the process of allocating a VkBuffer,
but the point is that there are several different configurations that
you may want.

\par Firstly, I generated a header-only class to simplify the
creation of a VkBuffer into one constructor call (albeit, I am unsure
if Gemini introduced any bugs in this code)

\par The next step I would think is to have another class that gives
me several options for generating the config structs, so that I don't
need to manually write them out every time.
% END CPP
...
\newglossaryentry{Execution Barrier}
{
name={Execution Barrier},
description={Idk}
}
\newglossaryentry{srcStageMask}
{
name={srcStageMask},
description={idk}
}
\newglossaryentry{dstStageMask}
{
name={dstStageMask},
description={idk}
}
\newglossaryentry{oldImageLayout}
{
name={oldImageLayout},
description={idk}
}
\newglossaryentry{newImageLayout}
{
name={newImageLayout},
description={idk}
}
\newglossaryentry{vkAccessFlags}
{
name={vkAccessFlags},
description={idk}
}
\newglossaryentry{GPU Hang}
{
name={GPU Hang},
description={idk}
}
\newglossaryentry{Pipeline State Object PSO}
{
name={Pipeline State Object PSO},
description={idk, but replaced with shader objects}
}
\newglossaryentry{Shader Object}
{
name={Shader Object},
description={idk}
}
...
\par It may be helpful to recall how we use descriptor sets.
\end{tcolorbox}
% END ANALYSIS

\section*{Deciding If I Should Stick With
Sascha}\addcontentsline{toc}{section}{Deciding If I Should Stick With Sascha}
\par Apparently, there are some outdate aspects of Sascha's Vulkan
Repo, so I think it is important to discuss whether or not I should
stick with it.

\par Here is an article about some modern Vulkan code:
\href{https://medium.com/@allenphilip78/using-modern-vulkan-in-2025-0bac45174304}{blog
post on modern Vulkan}

\par Here is a new resource for Vulkan Samples that I am considering:
\href{https://github.com/KhronosGroup/Vulkan-Samples}{Vulkan-Samples}

\par So what is different about these two resources? Well, the
Vulkan-Samples official repo use the newest Vulkan features, such as
synchronization 2.

\par Though, there are some trade-offs. Vulkan-Samples does not have
a lot of the built in functionality that Sascha's repo gives. I
wonder how hard it would be to port some of the functionality? I
wonder if it is possible. If there are any base classes that are not
cleanly separated from the Vulkan API, then they are probably a lost cause.

\par I'm also not sure if Vulkan-Samples has a lot of the struct
groupings that Sascha's repo has.
% END SWITCH

\section*{Modifying Triangle (from
vulkan-samples)}\addcontentsline{toc}{section}{Modifying Triangle
(from vulkan-samples)}
\subsection*{Loading a Model}\addcontentsline{toc}{subsection}{Loading a Model}
\par It feels logical to implement model loading first, maybe it
isn't \textit{too} hard yet and will teach me some basics.

\par First I modified two different CMakeLists.txt. This was so that
we could reduce compile-time per Gemini Pro's suggestion.

\par The second change was to add it to target\_link\_libraries and
that was literally adding one line (in root/framework/CMakeLists.txt)

\par After making those two minor changes my build is still working
so I think we are in the clear

\par Oh, and then I locked assimp's version like this:
\begin{verbatim}
git fetch --tags
git tag -l
cd ../..
git add external/assimp
git commit -m "Locked Assimp to stable version v5.4.3" //arbitray version number
\end{verbatim}

\par So what is the syntax for importing a model using assimp? There
are some more steps before we can do that, though.

\par Funnily enough after all of that, assimp is unecessary. We
already have tinygltf, which works with gltf\textemdash a format that
is much better in a game engine than other formats used for editing like fbx.

\par I think I will just comment it out in CMake for now, and leave
the dependency in third\_party

% END 2 MODIFY

\subsection*{Examining What We Already
Have}\addcontentsline{toc}{subsection}{Examining What We Already Have}
\par After that brief excursion, I am rethinking how I should
approach this repo.

\par The thing is, there is already a lot of boilerplate
implementation out of the way. I am just unsure how much time I
should spend looking for a starting point?

\par Or maybe that is the wrong way to think of it. Instead, I can
keep rethinking until I find something that sticks\textemdash that makes sense.

\par So, what don't we have? The two big ones are:
\begin{itemize}
\item Sound
\item Physics
\end{itemize}

\par For sound Gemini recommends:
\href{https://github.com/mackron/miniaudio}{Miniaudio}
\par For physics Gemini recommends:
\href{https://github.com/jrouwe/JoltPhysics}{JoltPhysics}

\par The other question is: which sample should I start from? I could
just pick something I know I need, like some sample loading a model.

\par Alternatively I could keep going with the triangle
sample\textemdash this way I know all of the changes I have made.

\par Once more, I could pick the sample I think has the most
functionality already implemented. Then of course there could be some
parts that I would want to remove.

\par Ultimately, I should just look around and see what I think. The
good part is I can probably rule out reading articles to decipher
some fundamental setup.

\subsection*{Trying To Understand Existing Camera
Implementation}\addcontentsline{toc}{subsection}{Trying To Understand
Existing Camera Implementation}
\par From what it seems, when we have a camera it writes to
descriptors, and that is how it affects the draw calls.

\subsection*{Analyzing Instancing
Sample}\addcontentsline{toc}{subsection}{Analyzing Instancing Sample}
\par So I am trying to understand this sample, but it uses render
pass, which is outdated functionality.

\par I want to learn modern vulkan, so if I am going to start from
this sample, then I need to re-design all of the old synchronization
stuff and re-design the renderpass stuff.

\par While researching I came across the idea of RAII, once more. I
keep seeing this idea everywhere, but I don't understand what it is
and why it's important.

\par I should really try to abstract vulkan to some classes and
interfaces, that way I can actually start making a game? I don't
really understand what is boilerplate that I can abstract and what's not

\par Some topics that I got to that I want to explore more:
\begin{enumerate}
\item RAII wrapper for instance.cpp stuff
\item Submitting to render queue
\item Persistant Scene Graph
\item Separating one-time boilerplate from every-frame update code.
\item Minimizing pipeline switches
\end{enumerate}
