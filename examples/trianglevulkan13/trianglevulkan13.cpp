...
// We want to keep GPU and CPU busy. To do that we may start building a new command buffer while the previous one is still being
// executed This number defines how many frames may be worked on simultaneously at once Increasing this number may improve
// performance but will also introduce additional latency
...
public:
  ...
  // exceptions to this not discussed here) Even though this adds a new dimension of planning ahead, it's a great opportunity for
  // performance optimizations by the driver
  VkPipeline pipeline{VK_NULL_HANDLE};

  // The descriptor set layout describes the shader binding layout (without actually referencing descriptor)
  // Like the pipeline layout it's pretty much a blueprint and can be used with different descriptor sets as long as their layout
  // matches
  ...
  void createCommandBuffers() {
    ...
    VkCommandBufferAllocateInfo cmdBufAllocateInfo =
        vks::initializers::commandBufferAllocateInfo(commandPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, MAX_CONCURRENT_FRAMES);
    VK_CHECK_RESULT(vkAllocateCommandBuffers(device, &cmdBufAllocateInfo, commandBuffers.data()));
  }

  // Prepare vertex and index buffers for an indexed triangle
  // Also uploads them to device local memory using staging and initializes vertex input and attribute binding to match the vertex
  // shader
  void createVertexBuffer() {
    ...
    // Setup indices
    // We do this for demonstration purposes, a triangle doesn't require indices to be rendered (because of the 1:1 mapping), but
    // more complex shapes usually make use of indices
    ...
    // Map the buffer and copy vertices and indices into it, this way we can use a single buffer as the source for both vertex and
    // index GPU buffers
  ...
  // Decriptors are used to pass data to shaders, for our sample we use a descriptor to pass parameters like matrices to the
  // shader
  void createDescriptors() {
    // Descriptors are allocated from a pool, that tells the implementation how many and what types of descriptors we are going to
    // use (at maximum)
    ...
    // Descriptors that are changed per frame need to be multiplied, so we can update descriptor n+1 while n is still used by the
    // GPU, so we create one per max frame in flight
    for (uint32_t i = 0; i < MAX_CONCURRENT_FRAMES; i++) {
  ...
  // While we don't do any depth testing in this sample, having depth testing is very common so it's a good idea to learn it from
  // the very start
  ...
  void createPipeline() {
    ...
    // Most states are baked into the pipeline, but there is somee state that can be dynamically changed within the command buffer
    // to mak e things easuer To be able to change these we need do specify which dynamic states will be changed using this
    // pipeline. Their actual states are set later on in the command buffer
  ...
  void createUniformBuffers() {
    ...
    for (uint32_t i = 0; i < MAX_CONCURRENT_FRAMES; i++) {
      ...
      // Note that we use the size we got from the memory requirements and not the actual buffer size, as the former may be larger
      // due to alignment requirements of the device
      ...
      VK_CHECK_RESULT(
          vkMapMemory(device, uniformBuffers[i].memory, 0, sizeof(ShaderData), 0, (void **)&uniformBuffers[i].mapped));
    }
  }
  ...
  void render() override {
    ...
    // Note that the implementation is free to return the images in any order, so we must use the acquire function and can't just
    // cycle through the images/imageIndex on our own
    uint32_t imageIndex{0};
    VkResult result = vkAcquireNextImageKHR(device, swapChain.swapChain, UINT64_MAX, presentCompleteSemaphores[currentFrame],
    ...
    // Copy the current matrices to the current frame's uniform buffer. As we requested a host coherent memory type for the
    // uniform buffer, the write is instantly visible to the GPU.
    ...
    // With dynamic rendering we need to explicitly add layout transitions by using barriers, this set of barriers prepares the
    // color and depth images for output
    vks::tools::insertImageMemoryBarrier(
        commandBuffer, swapChain.images[imageIndex], 0, VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
        VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
        VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, VkImageSubresourceRange{VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1});
    vks::tools::insertImageMemoryBarrier(
        commandBuffer, depthStencil.image, 0, VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
    ...
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1,
                            &uniformBuffers[currentFrame].descriptorSet, 0, nullptr);
    ...
    vks::tools::insertImageMemoryBarrier(commandBuffer, swapChain.images[imageIndex], VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, 0,
